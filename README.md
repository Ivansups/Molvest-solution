...
Сценарий 1: входящие сообщения открытой линии портала доходят до агента, а
ответ отправляется обратно в тот же диалог через REST. Полный рецепт прав и
наполнения — в [`docs/SCENARIOS.md`](docs/SCENARIOS.md) (сценарий 1).
...
| `BITRIX_LINE_ID` | линия открытой линии (опционально) |

Права приложения (локаль приложения в Bitrix24): `imconnector`
(подключение к открытой линии), `imopenlines` (чтение/отправка в линиях),
`disk` (выгрузка вложений при их скачивании через REST).

Регистрация входящего вебхука:

1. В локаль приложения установить права выше и получить `application_token`.
2. В настройках коннектора открытой линии указать URL вебхука
   `https://<наш-хост>/webhook/bitrix/openlines` (события входящих сообщений
   приходят на него).
3. Заполнить `BITRIX_*` в `.env`, перезапустить API.

Проверка: сообщение из диалога открытой линии на портале проходит `open`-диалог
агента, ответ приходит в тот же диалог; повторная доставка того же события
даёт `status: "duplicate"` без повторного запуска графа. Секреты ни в UI, ни в
`/settings`, ни в логи не попадают.
