...
ТЗ не требует единственного вендора LLM — OpenRouter здесь осознанный
выбор для быстрых шагов классификации (детект хэндоффа, GOOD/BAD кейса
в case learning), а не отклонение от требований.
...
| 2. Подсказка оператору | после эскалации в `draft`/`agent` бот молчит гостю, черновик в `/operator` |
| 3. Скриншот 1С | картинка → Vision → тот же поиск и порог, что у текста |
| 4. База знаний | админка: загрузка документов, чанки, эмбеддинги; плюс автоинжест решённых кейсов после `resolve` |

Порог (~80% similarity-скора retrieval'а, не самооценка модели) и явная
просьба «нужен человек» — два пути к оператору. Гостю при эскалации:
«Вопрос передан оператору техподдержки.» В Bitrix гостю тишина, черновик —
оператору.
...
flowchart TD
  ...
  B -->|нет| D{"status=escalated?"}
  D -->|нет / новый чат| G["история → LangGraph"]
  D -->|да, режим draft| E["реплика гостя, граф не запускается"]
  D -->|да, режим agent| RA{"повтор «Позовите оператора»?"}
  RA -->|да| E
  RA -->|нет| E2["реплика гостя + пересчёт черновика, граф не запускается"]
  E --> F["фраза: вопрос передан оператору"]
  E2 --> F
  G --> H{"граф: escalated?"}
  H -->|да и диалог уже escalated| E
  H -->|да, первый раз| I["open → escalated, тикет, гостю фраза"]
  H -->|нет, режим agent| AH["ответ ИИ удержан как черновик, гостю фраза"]
  H -->|нет, не agent| J["ответ GigaChat + источники"]
  I --> K["ChatResponse"]
  AH --> K
  J --> K
  F --> K
...
Bitrix и Redmine после commit считают черновик сами. В режиме `agent`
хэндофф удерживает готовый ответ ИИ как черновик вместо обычной реплики
(`apply_operator_reply_policy` в `app/services/agent.py`).
...
flowchart TD
  ...
  TH -->|да| generate["GigaChat generate, 40 с (таймаут)"]
