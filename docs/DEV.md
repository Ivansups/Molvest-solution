...
Запуск, переменные, Make и типичные поломки. Зачем проект и как думает
агент — в [README](../README.md). Каналы: [BITRIX.md](BITRIX.md),
...
| --- | --- | --- | --- |
| Docker Desktop | Compose v2 (`docker compose`) | API + Postgres + UI | [документация Docker](https://docs.docker.com/get-started/get-docker/) |
| uv | любой свежий | Python, **не** pip/poetry | [astral.sh/uv](https://docs.astral.sh/uv/getting-started/installation/) |
| Python | ≥ 3.12 | ставит сам uv (`server/.python-version`) | через uv |
| pnpm | 11.x (`packageManager` в `frontend/package.json`) | фронтенд, **не** npm/yarn | `corepack enable && corepack prepare pnpm@11.17.0 --activate` |
...
Шаблон — [`.env.example`](../.env.example). Живой файл — `.env` в **корне**
репозитория (его читает Compose и, при запуске из `server/`, бэкенд).
Полный список каналов — в шаблоне; ниже то, без чего спотыкаются чаще всего.

| Переменная | Обязательна сейчас | Комментарий |
| --- | --- | --- |
| `GIGACHAT_API_KEY` | нет для `/health` | без ключа агент не ответит по сути |
| `GIGACHAT_TIMEOUT` | нет | generate / Vision, по умолчанию 40 с |
| `GIGACHAT_CLASSIFY_TIMEOUT` | нет | YES/NO хэндоффа через GigaChat, 8 с |
| `OPENROUTER_API_KEY` | нет | пустой — детект «позовите оператора» идёт через GigaChat-2 |
| `OPENROUTER_TIMEOUT` | нет | YES/NO на OpenRouter, 5 с |
...
| `OPERATOR_ASSIST_MODE` | нет | `auto` (по умолчанию), `draft` или `agent` — см. [OPS.md](OPS.md#порог-и-режимы) |
...
| `AUTH_DATABASE_URL` | да для входа в UI | та же БД драйвером Prisma, схема `auth` |
...
Линтеры, тесты и сборка образов гоняются в GitHub Actions на каждый PR и
push в `main` (`.github/workflows/ci.yml`).

Миграции гоняет entrypoint контейнера API перед uvicorn. `make up` не
завершится, пока `/health` не ответит. `make migrate` нужен, только если
применили новую ревизию без перезапуска контейнера.
...
WSL или Git Bash, либо шаги [без Make](#без-make).
