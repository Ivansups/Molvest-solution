...
## Автоинжест кейсов (case learning)

После `resolve` эскалированного диалога фоново запускается
`evaluate_and_ingest` (`app/services/case_learning.py`): правила отбирают
пару «вопрос гостя — автоответ ассистента» без эскалации и выше порога
уверенности, малая модель через `OPENROUTER_API_KEY` выносит GOOD/BAD, и
только GOOD индексируется как документ `FileType.KB_CASE`.

Без `OPENROUTER_API_KEY` (пустая переменная) вся эта цепочка **молча
пропускается** для каждого `resolve` — не ошибка, не 5xx, только INFO-лог
`case learning: OpenRouter не настроен conversation_id=...`. Снаружи это
неотличимо от "кейсов просто пока не было" — если автоинжест не работает,
первым делом проверьте, что ключ вообще задан, а не искать баг в правилах
или в индексации.

Fail-closed: сбой сети к OpenRouter или ошибка индексации (эмбеддинг-сервис
недоступен и т.п.) не помечают диалог разобранным — следующий `resolve`
этого же диалога повторит попытку. BAD-вердикт или несовпадение с
правилами — диалог помечается разобранным сразу, повтора не будет.

Настраивается `case_learning_enabled` (дефолт `true`, пока без экспозиции
в `PUT /api/settings` — только через конфиг).

## Перед публичным демо

Дефолты `.env.example` рассчитаны на локальную разработку и **не годятся**
для хоста, доступного из интернета:

- `INTERNAL_SERVICE_TOKEN` — задать непустое значение. Пустое отключает
  проверку `X-Internal-Token` целиком (см. [service-to-service-auth](
  ../openspec/specs/service-to-service-auth/spec.md)), и все domain-роуты
  становятся доступны без авторизации.
- `POSTGRES_USER` / `POSTGRES_PASSWORD` — заменить дефолтные
  `postgres`/`postgres` на уникальные значения.
- Не публиковать порт Postgres (`5432`) наружу — только внутри
  docker-сети/приватной подсети.
- Bitrix install-эндпоинт (`POST /webhook/bitrix/install`, см.
  [BITRIX.md](BITRIX.md)) не проверяет подпись запроса — единственный
  барьер сейчас это секретность `BITRIX_HANDLER_BASE_URL`. Не публикуйте
  этот URL там, где его может подобрать посторонний.
