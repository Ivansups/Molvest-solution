...
The system SHALL read operator assist mode from one configuration setting `OPERATOR_ASSIST_MODE` with values `draft` or `auto`. The default SHALL be `draft`. The setting SHALL NOT be hardcoded in graph nodes or UI. There is no public settings API for this value.
...
- **WHEN** `OPERATOR_ASSIST_MODE` is `auto` and an already escalated conversation receives a guest support question whose retrieval score is at or above the confidence threshold
